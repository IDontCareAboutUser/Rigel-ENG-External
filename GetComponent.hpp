...
/*inline uint64_t DecryptComponent(uint64_t parent, uint8_t idx)
{
	...
	inline uintptr_t DecryptComponent(uintptr_t parent, uint8_t idx) {
		__try {
			if (parent) {
				...
				unsigned __int64 Key1 = SDK->GlobalKey1;
				unsigned __int64 Key2 = SDK->GlobalKey2;
				auto dummy = SDK->RPM<_QWORD>(SDK->dwGameBase + 0x3807620 + (Key1 >> 0x34));
				auto dummy2 = SDK->RPM<_QWORD>(SDK->dwGameBase + 0x3807620 + (Key1 & 0xFFF));
				unsigned __int64 v10 = Key2 ^ ((unsigned int)v9 | ((unsigned int)v9 | v9 & 0xFFFFFFFF00000000ui64 ^ ((unsigned __int64)(unsigned int)(2 * __ROR4__(dummy, 3) - v9) << 0x20)) & 0xFFFFFFFF00000000ui64 ^ ((unsigned __int64)(__ROR4__(HIDWORD(dummy2), 0xB) ^ (unsigned int)~(_DWORD)v9) << 0x20));
				unsigned __int64 v11 = (unsigned int)v10 | v10 & 0xFFFFFFFF00000000ui64 ^ ((unsigned __int64)((unsigned int)v10 ^ 0x281677CD) << 0x20);
				unsigned __int64 v12 = -(int)v7 & (unsigned __int64)((unsigned int)v11 | v11 & 0xFFFFFFFF00000000ui64 ^ ((unsigned __int64)((unsigned int)v10 ^ HIDWORD(dummy)) << 0x20));
				return v12;
			}
		}
		__except (EXCEPTION_EXECUTE_HANDLER) {}
	...
	inline unsigned __int64 __fastcall DecryptVis(__int64 a1) {
		...
		v3 = v2 + 0x8;
		v4 = SDK->RPM<uint64_t>(SDK->dwGameBase + 0x3808640
			+ 8 * (((uint8_t)a1 + 0x5A) & 0x7F)
		...
		for (; v6 < v5; ++v6) {
			v4 ^= SDK->RPM<uint64_t>(v2);
			v2 += 8i64;
		}
		return v4 ^ ~v3 ^ offset::Vis_Key;
	}
	...
	int64_t DecryptOutline(uint64_t a1)
	{
		...
		v3 = v2 + 0x8;
		v4 = 0i64;
		v5 = SDK->RPM<uint64_t>(SDK->dwGameBase + 0x3808640
			+ 8ull * (((unsigned __int8)a1 + 0x31) & 0x7F)
		...
		for (; v2 < v3; v2 += 8i64)
			v5 ^= SDK->RPM<uint64_t>(v2);
		return v5 ^ ~v3 ^ 0xDF9DC69B9E06D531ui64;
	}
